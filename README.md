...
1. 算子已经支持多 tiling 特化 kernel 一次编译、host 运行时 dispatch。
2. A 使用 vector-CSR 格式，当前要求 M 是 16 的倍数；N/K 非规则输入可以正确处理。
3. 在 4096³、95% 稀疏度下达到 `0.754697 ms / 9.105574 effective TFLOPS / 182.111435 dense-equivalent TFLOPS`。
4. 在 8192³、95% 稀疏度压力测试下达到 `6.391463 ms / 8.601408 effective TFLOPS / 172.028161 dense-equivalent TFLOPS`。
5. 算子更适合中大规模、大 N、中高稀疏度场景。

### 14.2 后续优化方向

1. **tiling 阈值优化**：当前阈值在非零向量数接近切换边界时可能存在性能断点，可以进一步用自动搜索或 profile 反馈调优。
2. **small-N 专用路径**：N 很小的长方体 case 性能偏低，可考虑单独 small-N kernel 或减少 B-pack 固定开销。
3. **极高稀疏度优化**：98%/99% 下 dense-equivalent 很高，但 effective TFLOPS 下降，可进一步减少固定同步与 pack 开销。
4. **M-tail 通用化**：当前数据生成和格式要求 M 为 16 的倍数。若业务需要任意 M，需要补充 M 方向尾块处理。
